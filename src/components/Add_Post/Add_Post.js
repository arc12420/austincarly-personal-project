...
function Add_Post (props) {
    ...
    function cDM(){
        if (props.match.params.id) {
            axios
              .get(`/api/getPost/${props.match.params.id}`)
              .then((res) => {
                  setEdit (true)
        ...
        <div className="addPostComponent">
          <main className="addPostBox">
            ...
            <div className="addPostButtons">
              {!edit ? (
              <button className="AddPButtons" onClick={makePost}>Post</button>
              ) : (
                <button className="AddPButtons" onClick={updatePost}> Save </button>
              )}
              <button className="AddPButtons" onClick={() => props.history.push("/Home")}>
                Cancel
              </button>
            </div>
          </main>
